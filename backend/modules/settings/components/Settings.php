...
use backend\modules\settings\models\Settings as SettingModel;
use Yii;
...
class Settings extends Component
{
    ...
    public function get($section, $key, $default = null)
    {
        ...
        if (isset($data[$section][$key][0])) {
            ...
            if ($data[$section][$key][2]){

                try {
                    $value = Json::decode($value);
                    if (is_array($value) && isset($value[Yii::$app->language]) ){
        ...
        if ($this->_data === null) {
            if ($this->cache instanceof Cache) {
                ...
                if ($data === false) {
                    ...
                    $this->cache->set($this->cacheKey, $data, null, new TagDependency(['tags' => 'settings']));
                }
            } else {
                $data = $this->model->getSettings();
            }
